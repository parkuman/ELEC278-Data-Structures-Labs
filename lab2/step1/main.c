...
// don't do anything
{
    ...
    // check if given values exist
    if (l == NULL)
    {
        fprintf(stderr, "ERROR: No array or x value");
    }

    // if array is full, remove last entry so we can shift everything over to fit a new entry at the front
    if ((l->end + 1) == l->size)
    {
        removeEnd(l);
    }

    // loop through array and shift values one space forward
    ...
    // make the first element in a the value we want to insert
    l->a[0] = x;
    //increase end by 1
    l->end++;
} //addfront()
...
int main()
{
    ...
    printf("ELEC278 - Lab2 Part1 arraylist program\n");
    init(&ll, 10);
